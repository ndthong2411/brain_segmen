# BrainTumNet - Brain Tumor Segmentation & Classification

## Overview
BrainTumNet is a PyTorch-based deep learning framework for multi-task brain tumor analysis on BraTS2020 MRI data. The system performs both semantic segmentation (tumor region identification) and classification (HGG vs LGG grading) using a novel architecture combining U-Net, CBAM attention, and Adaptive Masked Transformer modules.

### Key Features
- **Multi-task Learning**: Simultaneous segmentation and classification
- **Advanced Architecture**: U-Net + CBAM + Adaptive Masked Transformer
- **HDF5 Support**: Direct processing of BraTS2020 HDF5 format
- **Comprehensive Pipeline**: End-to-end from preprocessing to inference
- **Production Ready**: TensorBoard logging, checkpointing, visualization
...
│   ├── default.yaml           # Full training config (250 epochs)
│   └── quick_test.yaml        # Fast testing config (3 epochs)
├── data/                      # Dataset storage (git-ignored)
│   ├── raw/                   # Original BraTS2020 data
│   └── processed/             # Preprocessed PNG slices + splits
